...
class Parser:
    def parse_main_topics(self):
        ...
        for i in main_topics:
            ...
            self.serialization(d,SAVE_NAMES["main_topics"])
            print_warning_text("main topics has been update")



    def parse_sub_topics(self):
        p = pq(HTTP_MAIN_PAGE)
        ...
        for topic in sub_topics:
            ...
            self.serialization(d,SAVE_NAMES["sub_topics"])
            print_warning_text("sub topics has been update")

        
    def parse_post(self,child):
        """
    ...
    def get_pages(self,id):
        ...
        counter_page = 1
        while True:
            page = self.get_posts(self.get_page(id,counter_page))
            if page:
                counter_page+=1
                yield page
            else:
